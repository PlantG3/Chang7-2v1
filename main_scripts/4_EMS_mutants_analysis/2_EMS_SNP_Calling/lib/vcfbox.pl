...
sub genomatch {
	...
	[arguments]
		...
		-o: output vcf file; xxx.genomatch.vcf will be generated if -o is not specified
	\n/) if (@ARGV==0 && -t STDIN);
	...
	print STDERR "Note: $output is generating.\n";
...
sub recall {
	my %opts = ();
	&getopts('A:a:B:b:R:r:N:n:s:m:o:', \%opts);
	my $homoMinAD = (defined $opts{A}) ? $opts{A} : 4;
	my $homoMinADperc = (defined $opts{a}) ? $opts{a} : 0.9;
	my ($heteroREFminAD, $heteroREFminADperc);
	my ($heteroNonREFminAD, $heteroNonREFminADperc);
	
	# hetero counts:
	if (defined $opts{B}) {
		$heteroREFminAD = $opts{B};
		$heteroNonREFminAD = $opts{B};
	} elsif (defined $opts{R} or defined $opts{N}) {
		if (defined $opts{R}) {
			$heteroREFminAD = $opts{R};
		} else {
			$heteroREFminAD = 1;
		}
		
		if (defined $opts{N}) {
			$heteroNonREFminAD = $opts{N};
		} else {
			$heteroNonREFminAD = 1;
		}
	} else {
	...
	# hetero percentage
	if (defined $opts{b}) {
		$heteroREFminADperc = $opts{b};
		$heteroNonREFminADperc = $opts{b};
	} elsif (defined $opts{r} or defined $opts{n}) {
		if (defined $opts{r}) {
			$heteroREFminADperc = $opts{r};
		} else {
			$heteroREFminADperc = 0.2;
		}
		
		if (defined $opts{n}) {
			$heteroNonREFminADperc = $opts{n};
		} else {
			$heteroNonREFminADperc = 0.2;
		}
	} else {
		$heteroREFminADperc = 0.2;
		$heteroNonREFminADperc = 0.2;
	...
	[Note]: -b overrides -r and -n
	-R: minimum counts of REF allele for heterozygous genotype calls (integer >0) (1)
	-r:	minimum percentage of counts of REF allele out of read depth of the site for heterozygous genotype calls (float 0-1) (0.2)
	-N: minimum counts of nonREF allele for heterozygous genotype calls (integer >0) (1)
	-n: minimum percentage of counts of nonREF allele out of read depth of the site for heterozygous genotype calls (float 0-1) (0.2)
	-s: alleleseparator, either | or \/; ("\/")
	-m: maximum missing rate after genotype recalling (float 0-1); (1)	
	-o: output vcf file; if no output file is specified, it will be automatically generated ended with ".recall.vcf"
	...
	print STDERR "min REF depth to call hetero: $heteroREFminAD\n";
	print STDERR "min REF depth % to call hetero: $heteroREFminADperc\n";
	print STDERR "min NonREF depth to call hetero: $heteroNonREFminAD\n";
	print STDERR "min NonREF depth % to call hetero: $heteroNonREFminADperc\n";
			...
			for (my $k=9; $k<=$#t; $k++) {
				...
				if (exists $fd{AD}) {
					...
					if ($ad ne "NA") {
						...
						# recall
						$newgeno = &genocall($missing_geno, $alleleseparator, $ad[0], $ad[1], $dp, $homoMinAD, $homoMinADperc, $heteroREFminAD, $heteroREFminADperc, $heteroNonREFminAD, $heteroNonREFminADperc);
	...
	# recall genotype based on counts of alleles
		...
		my ($geno, $sepchar, $ad1, $ad2, $tdp, $homoAD, $homoADp, $heteroREFAD, $heteroREFADp, $heteroNonREFAD, $heteroNonREFADp) = @_;
		# homo 1
		...
		# hetero
		if ($ad1 >= $heteroREFAD and $ad2 >= $heteroNonREFAD and
		    ($ad1 / $tdp) >= $heteroREFADp and ($ad2 / $tdp) >= $heteroNonREFADp) {
			$geno = "0".$sepchar."1";
		}
		return $geno;
	}
...
sub sitefilt {
	my %opts = ();
	&getopts('v:m:P:p:r:R:a:A:H:h:o:', \%opts);
	...
	my $REFhomoPercMin = (defined $opts{r}) ? $opts{r} : 0;
	my $REFhomoPercMax = (defined $opts{R}) ? $opts{R} : 1;

	my $ALThomoPercMin = (defined $opts{a}) ? $opts{a} : 0;
	my $ALThomoPercMax = (defined $opts{A}) ? $opts{A} : 1;
	...
	[arguments]
		-v: minimum datapoints per site with non-missing genotype (integer, >0); default=1
		-m: maximum missing rate allowed (float 0-1) (1)
		-p: minimum homozygous rate for each allele (float 0-0.5); (0)
		-P: maximum homozygous rate for each allele (float 0-1); (1)
		-r: minimum homozygous rate for REF allele (float 0-1); (0)
		-R: maxmum homozygous rate for REF allele (float 0-1); (1)
		-a: minimum homozygous rate for ALT allele (float 0-1); (0)
		-A: maxmum homozygous rate for ALT allele (float 0-1); (1)
	...
	print STDERR "- minimum homozygous rate of each allele: $homoPercMin\n";
	print STDERR "- maximum homozygous rate of each allele: $homoPercMax\n";
	print STDERR "- minimum homozygous rate of REF allele: $REFhomoPercMin\n";
	print STDERR "- maximum homozygous rate of REF allele: $REFhomoPercMax\n";
	print STDERR "- minimum homozygous rate of ALT allele: $ALThomoPercMin\n";
	print STDERR "- maximum homozygous rate of ALT allele: $ALThomoPercMax\n";
		...
		if (!/\#\#/) {
			...
			if ($nvalid >= $minValid) {
				...
				if ($missing_perc <= $maxMissingPerc and
					...
					$homo1_perc >= $REFhomoPercMin and $homo1_perc <= $REFhomoPercMax and
					$homo2_perc >= $homoPercMin and $homo2_perc <= $homoPercMax and
				    $homo2_perc >= $ALThomoPercMin and $homo2_perc <= $ALThomoPercMax and
...
######################################################################### 
sub taxafilt {
	#my %opts = (v=>100, m=>1, f=>0, c=>1, o=>"test");
	my %opts = ();
	&getopts('v:m:f:c:o:', \%opts);
	my $minValid = (defined $opts{v}) ? $opts{v} : 100;
	my $maxMissingPerc = (defined $opts{m}) ? $opts{m} : 1;
	...
	[arguments]
		-v: minimum datapoints\/sites with non-missing genotype (integer); default=100
		-m: maximum missing rate allowed (float 0-1) (1)
